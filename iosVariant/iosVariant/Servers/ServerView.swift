        ...
        VStack {
            if self.variantViewModel.editing {
                ...
                BrowseServerView(
                    ...
                    contents: self.variantViewModel.directoryContents,
                ...
                ServerListView(
                    ...
                    onBrowseServer: { server in
                        Log().info(tag: TAG, message: "Starting to browse server: \(server.name)")
                        Task {
                            try await self.variantViewModel.loadDirectory(server: server, path: ReaderAPIKt.READER_ROOT, reload: false )
                        }
                    }
                )
            }
